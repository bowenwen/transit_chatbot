    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if '200' in str(resp):  #valid response code
            ...
            dispatcher.utter_message(
                "Your next bus will arrive at: {}".format(lvtime))
        else:
            dispatcher.utter_message(
...
#define function to retrieve directions
    def direction(self, location_start, location_end, departure_time):
        '''
        ...
        url = self.url_base + encoded_args
        encoded_url = url + '&key=' + self.googlemaps_api_key
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if location_start == '' or location_end == '' or location_start is None or location_end is None or location_start == location_end:
            ...
            data = self.direction(location_start, location_end,
                                  self.departure_time)
            ...
            if data['status'] != 'OK':
                ...
                start_address = self.remove_postalcode(start_address).replace(
                    ', BC', '').replace(', Canada', '')
                end_address = self.remove_postalcode(end_address).replace(
                    ', BC', '').replace(', Canada', '')

                #create a map url
                map_url = 'https://www.google.com/maps?' + urllib.parse.urlencode(
                    {
                        'saddr': start_address,
                        'daddr': end_address,
                        'dirflg': 'r'
                ...
                if travel_time == 0:
                    ...
                    arrival_time = data['routes'][0]['legs'][0][
                        'arrival_time']['text']
                ...
                num_iters = max(
                    ...
                    len(data['routes'][0]['legs'][0]['steps'][0]['steps']))
                for i in range(0, num_iters):
                    try:
                        instructions.append(
                            self.remove_postalcode(
                                data['routes'][0]['legs'][0]['steps'][i]
                                ['html_instructions'].replace(
                                    'Subway towards',
                                    'Take SkyTrain')).replace(
                                        ', BC', '').replace(
                                            ', Canada', '').replace(
                                                'U-turn', 'turn'))
                    except:
                    ...
                    for j in range(0, num_iters):
                        try:
                            instructions.append(
                                self.remove_postalcode(
                                    self.remove_htmltags(
                                        data['routes'][0]['legs'][0]['steps']
                                        [i]['steps'][j]['html_instructions'])).
                                replace(', BC',
                                        '').replace(', Canada', '').replace(
                                            'U-turn', 'turn'))
                        except:
                            pass

                dispatcher.utter_message(
                    "Here are the directions from {} to {}:\n{}\n\nIf you leave now, you should arrive at {} by {}. Your trip should take around {}.\n"
                    .format(start_address, end_address,
                            ("\n".join(instructions)), end_address,
                            arrival_time, travel_time))
                dispatcher.utter_message(
                    "Click on the following link to view the direction on Google Maps:\n{}"
                    .format(map_url))
